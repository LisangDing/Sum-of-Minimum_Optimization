...
for n = 1:N_trial
    ...
    [A_EM, loss_EM] = EM(x, A0, iter_max);

    A0_us = uniform_seeding(x, K, r); % uniform index initialization
    [A_EMus, loss_EMus] = EM(x, A0_us, iter_max);

    A0_cs = careful_seeding(x, K, r);
    [A_EMcs, loss_EMcs] = EM(x, A0_cs, iter_max);
...
%% EM algorithm
function [A, loss] = EM(x, A0, iter_max)
    A = A0;
    ...
    while true
        ...
        min_loc_old = min_loc;
        for k = 1:K
            X = 0.5 * x(min_loc == k,:)' * x(min_loc == k,:);
            [V, D] = eig(X);
            % Extract the diagonal elements (eigenvalues) and sort them
            [eigenvalues, index] = sort(diag(D));
            
            % Select the first r indices (corresponding to the smallest eigenvalues)
            r_indices = index(1:r);
            
            % Select the eigenvectors corresponding to the r smallest eigenvalues
            A(k,:,:) = V(:, r_indices);
        end
        iter = iter + 1;
    end
